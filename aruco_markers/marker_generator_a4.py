...
Description
-----------
Generates a PDF file with ArUco markers arranged on a grid for printing on A4 paper.
...
def main():
    ...
    for i in range(num_markers):
        marker_bits = np.random.randint(
            0, 2, (marker_size, marker_size), dtype=np.uint8
        )
        print(marker_bits)
        ...
        dictionary.bytesList = np.append(
            dictionary.bytesList, marker_compressed, axis=0
        )

    print(
        f"✅ Custom dictionary created with {num_markers} markers of {marker_size}x{marker_size} bits."
    )
